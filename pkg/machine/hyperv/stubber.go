...
func (h HyperVStubber) PostStartNetworking(mc *vmconfigs.MachineConfig, _ bool) error {
	...
	if logrus.IsLevelEnabled(logrus.DebugLevel) {
		log, err := logCommandToFile(fsCmd, "machine-server9p.log")
		if err != nil {
			return err
		}
		defer log.Close()
	...
	// Note: When using user mode networking, no callback is needed to stop the 9p server, because it will stop when
	// gvproxy stops. When using provider mode networking, we need to store the PID file to clean up on VM stop.
	if !mc.HyperVHypervisor.UserModeNetworking {
